...
public class Frame {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        context.fill(x, y, x + width, y + height, rgbColor());
        int offset = ((height / 2) - mc.textRenderer.fontHeight / 2);

        context.drawTextWithShadow(mc.textRenderer, category.name, x + offset, y + offset, -1);
        context.drawTextWithShadow(mc.textRenderer, extended ? "-" : "+", x + width - offset - 2 - mc.textRenderer.getWidth("+"), y + offset, -1);
    ...
    public void updateButtons() {
        ...
        for (ModuleButton button : buttons) {
            ...
            offset += height;
            if (button.extended) {
                for (Component ignored : button.components) {
